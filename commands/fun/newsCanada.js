...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('dailynews-canada')
    .setDescription('Get daily Canadian Business news headlines'),
  async execute(interaction) {
    const newsUrl = `https://newsapi.org/v2/top-headlines?country=ca&category=business&apiKey=${newsApi}`;
    ...
    try {
      ...
      if (newsData.status === 'ok' && newsData.articles.length > 0) {
        ...
        await interaction.reply(`**Top 5 Daily Canadian Business News Headlines:**\n\n${newsHeadlines.join('\n')}`);
      } else {
