...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('dailynews-business')
    .setDescription('Get daily news headlines'),
  async execute(interaction) {
    const newsUrl = `https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=${newsApi}`;
    ...
    try {
      ...
      if (newsData.status === 'ok' && newsData.articles.length > 0) {
        ...
        await interaction.reply(`**Top 5 Daily Business News Headlines:**\n\n${newsHeadlines.join('\n')}`);
      } else {
